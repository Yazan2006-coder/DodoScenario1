...
public class MyDodo extends Dodo
{
    ...
    /**
     * Adds parity bits to all rows and columns without using compass directions.
     */
    public void addParityBitsGeneric() {
    int totalRows = getWorld().getHeight();
    int totalCols = getWorld().getWidth();
    int currentRow = 0;
    while (currentRow < totalRows) {
        // ga naar begin van rij via borderAhead
        walkToWorldEdgePrintingCoordinates();   // loop naar rechts tot de rand
        turn180();           // draai om (nu naar links)
        walkToWorldEdgePrintingCoordinates();   // loop naar links tot de rand = begin rij
        // tel eieren in deze rij
        int eggs = countEggsInLine();
        if (eggs % 2 != 0) {
            // oneven ? gouden ei op huidige positie (einde rij)
            getWorld().addObject(new GoldenEgg(), getX(), getY());
        }
